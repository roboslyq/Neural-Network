...
"""
一维卷积实验2：三元特征序列检测
目标：使用一维卷积网络检测序列中的"010"模式
例如：输入序列"11010110"，输出在检测到上升后下降的位置标记1
"""

class Net1d(nn.Module):
    """一维卷积神经网络模型
    使用kernel_size=3的一维卷积来检测连续三个位置的模式
    """
    def __init__(self):
        ...
        # kernel_size=3: 卷积核大小为3，用于检测连续三个位置
    ...
    def forward(self, x):
        ...
        o4 = torch.sigmoid(o1)  # sigmoid激活函数
        return o4
...
def obj_func(in_tensor):
    ...
    检查每个位置i，如果满足：
    1. i+1位置的值大于i位置的值（上升）
    2. i+2位置的值小于i+1位置的值（下降）
    则输出1，否则输出0
    
    Args:
        in_tensor: 输入张量，形状为 [批次大小,1,序列长度]
    Returns:
        输出张量，形状为 [批次大小,1,序列长度-2]，标记了每个位置是否出现"010"模式
    """
    return torch.FloatTensor([
        [
            [
                1 if seq[i + 1] > x and seq[i + 2] < seq[i + 1] else 0  # 检测上升后下降的模式
